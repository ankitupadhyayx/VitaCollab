...
export default function TimelinePage() {
  ...
  const mapped = useMemo(() => {
    const transformed = records.map((item) => ({
        ...
        fileName: `${item.type || "medical-file"}-${item.id || "record"}`,
        fileLink: toAbsoluteApiUrl(item.fileAccessUrl || `/api/v1/files/${item.id}`),
        fileDownloadLink: toAbsoluteApiUrl(item.fileDownloadUrl || `/api/v1/files/${item.id}?download=true`),
